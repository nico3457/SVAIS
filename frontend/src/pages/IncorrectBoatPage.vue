<template>
  ...
  const boats=[{
    ...
    CallSign: "WDG9352",
    VesselType: "Remolque",
    Status: "En marcha utilizando el motor",
    Length: 30,
    Width: 10,
    Draft: 5,
    Cargo: 31,
  ...
  async function initializeMapAndLocator() {
    ...
    map= L.map('map').setView([42.2706, -8.7207], 13);
    ...
    boats.forEach(boat => {
      ...
      marker.on('click', async() => {
        ...
        const date = new Date(boat.BaseDateTime);
        const formattedDate = `${date.getDate()}-${date.getMonth() + 1}-${date.getFullYear()}`;
        const formattedTime = `${date.getHours()}:${date.getMinutes()}:${date.getSeconds()}`;
  
        var popupContent = `<div class="popup-content">
                                  <span class="label">Nombre:</span> <span class="boat-name">${boat.VesselName}</span><br>
                                  <span class="label">Vessel Type:</span> <span class="vessel-type">${boat.VesselType}</span><br>
                                  ...
                                  <span class="label">Estado:</span> <span class="status">${boat.Status}</span><br>
                                  <span class="label">Latitud:</span> <span class="latitude">${boat.LAT}</span><br>
                                  <span class="label">Longitud:</span> <span class="longitude">${boat.LON}</span><br>
                                </div>`;
